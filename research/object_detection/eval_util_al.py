...
def _run_checkpoint_once(tensor_dict,
  ...
  with tf.contrib.slim.queues.QueueRunners(sess):
    try:
      for batch in range(int(num_batches)):
        ...
        for evaluator in evaluators:
          ...
          evaluator.add_single_detected_image_info(
              image_id=batch, detections_dict=result_dict,prediction_dict=result_prediction_dict)
      ...
      for evaluator in evaluators:
        ...
        detections = evaluator.get_detections()
        groundtruths = evaluator.get_groundtruths()
        evaluator.clear()
  ...
  sess.close()
  return (global_step, all_evaluator_metrics,detections,groundtruths)
...
def repeated_checkpoint_run(tensor_dict,
  ...
  while True:
    ...
    if not model_path:
      ...
      global_step, metrics, detections, groundtruths = _run_checkpoint_once(tensor_dict, evaluators,
                                                  batch_processor,
                                                  checkpoint_dirs,
  ...
  return metrics,detections,groundtruths
