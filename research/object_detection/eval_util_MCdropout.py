      ...
      for batch in range(int(num_batches)):
        ...
        if not batch_processor:
          try:
            ...
            sel_prediction_dict = {}
            for k in range(1,11):
                result_dict, result_losses_dict,result_prediction_dict, result_detections = batch_processor(tensor_dict, sess, batch, counters, losses_dict=losses_dict,prediction_dict=prediction_dict,detections=detections)

                # Select necessary fields from prediction_dict and save it to file
                sel_prediction_dict['run_{:d}'.format(k)] =  result_prediction_dict['class_predictions_with_background']
            #sel_prediction_dict['refined_decoded_boxes'] = result_detections['refined_decoded_boxes']
            savemat("{}{:05d}.mat".format("/home/abel/DATA/faster_rcnn/resnet101_coco/results_DrpAllRuns/im-",batch),sel_prediction_dict)
        if not result_dict:
          continue
